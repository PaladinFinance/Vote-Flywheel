...
describe('HolyPalPower contract tests', () => {
    ...
    describe('locked__end', async () => {
        ...
        it(' should return the correct lock end', async () => {
            ...
            expect(await power.locked__end(TEST_USER)).to.be.eq(BigNumber.from(lock_1.startTimestamp).add(lock_1.duration))

            const lock_2 = await hPal.getUserLock(TEST_USER_2)
            expect(await power.locked__end(TEST_USER_2)).to.be.eq(BigNumber.from(lock_2.startTimestamp).add(lock_2.duration))

            const lock_3 = await hPal.getUserLock(TEST_USER_3)
            expect(await power.locked__end(TEST_USER_3)).to.be.eq(BigNumber.from(lock_3.startTimestamp).add(lock_3.duration))

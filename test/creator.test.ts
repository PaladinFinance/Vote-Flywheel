...
describe('LootCreator contract tests', () => {
    ...
    describe('notifyFixedQuestPeriod', async () => {
        ...
        const total_rewards = ethers.utils.parseEther("2500")
        const new_total_rewards = ethers.utils.parseEther("3100")
        const new_total_rewards2 = ethers.utils.parseEther("2250")
        ...
        it(' should update the storage corectly - increase', async () => {

            const prev_total_quest_period = await creator.totalQuestPeriodRewards(distributor.address, quest_id, period)

            await distributor.connect(admin).sendNotifyFixedQuestPeriod(creator.address, quest_id, period, new_total_rewards)

            const new_total_quest_period = await creator.totalQuestPeriodRewards(distributor.address, quest_id, period)

            expect(new_total_quest_period).not.to.be.eq(prev_total_quest_period)
            expect(new_total_quest_period).to.be.eq(new_total_rewards)

        });

        it(' should update the storage corectly - decrease', async () => {

            const prev_total_quest_period = await creator.totalQuestPeriodRewards(distributor.address, quest_id, period)

            await distributor.connect(admin).sendNotifyFixedQuestPeriod(creator.address, quest_id, period, new_total_rewards2)

            const new_total_quest_period = await creator.totalQuestPeriodRewards(distributor.address, quest_id, period)

            expect(new_total_quest_period).not.to.be.eq(prev_total_quest_period)
            expect(new_total_quest_period).to.be.eq(new_total_rewards2)
        ...
        it(' should fail if caller is not allowed', async () => {
            ...
            await expect(
                creator.connect(user1).notifyFixedQuestPeriod(quest_id, period, new_total_rewards)
            ).to.be.revertedWith("CallerNotAllowed")

            await expect(
                creator.connect(admin).notifyFixedQuestPeriod(quest_id, period, new_total_rewards)
            ).to.be.revertedWith("CallerNotAllowed")
